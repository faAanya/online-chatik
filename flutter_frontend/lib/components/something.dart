...
class InfiniteScrollPhysics extends ScrollPhysics {
  const InfiniteScrollPhysics({
    super.parent,
    this.snapStiffness = 900,
    this.snapDampingRatio = 1.15,
  });

  final double snapStiffness;
  final double snapDampingRatio;
  ...
  InfiniteScrollPhysics applyTo(ScrollPhysics? ancestor) {
    return InfiniteScrollPhysics(
      ...
      snapDampingRatio: snapDampingRatio,
    );
  }

  // Make the snap spring stronger than Flutter’s default to get a fast ease-out.
  @override
  SpringDescription get spring => SpringDescription.withDampingRatio(
    mass: 0.5,
    stiffness: snapStiffness,
    ratio: snapDampingRatio,
  );
  ...
  Simulation? createBallisticSimulation(
    ...
    // 1) Out of range and headed further out? Defer to parent to go back.
    if ((velocity <= 0.0 && metrics.pixels <= metrics.minScrollExtent) ||
        (velocity >= 0.0 && metrics.pixels >= metrics.maxScrollExtent)) {
      return super.createBallisticSimulation(metrics, velocity);
    }

    // Predict natural landing to decide which item to snap to.
    ...
    // 3) Already there with no velocity? Do nothing.
    if (velocity.abs() < tolerance.velocity &&
        (settlingPixels - metrics.pixels).abs() < tolerance.distance) {
      return null;
    }

    // 4) Not enough oomph to leave current item? Use spring back (already ease-out).
    ...
    // 5) Snap to the target item with a (stiff, critically/over-damped) spring.
    //    This produces the desired "ease-out" feel: fast at first, then tapering.
...
class PageViewTypeScrollPhysics extends ScrollPhysics {
  ...
  double _getTargetPixels(
    _InfiniteScrollPosition metrics,
    Tolerance tolerance,
    double velocity,
  ) {
